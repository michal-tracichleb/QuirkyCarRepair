...
export async function serviceOrderRepair(id, description) {
    const user = sessionStorage["user"] ? JSON.parse(sessionStorage["user"]) : [];
    try {
        const response = await axios.get(`${BACK_END_URL}/CarService/ServiceOrderRepair?id=${id}&description=${description}`, {
            headers: {
