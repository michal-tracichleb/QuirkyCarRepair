...
export async function serviceOrderAcceptedByClient(id, description) {
    const user = sessionStorage["user"] ? JSON.parse(sessionStorage["user"]) : [];
    try {
        const response = await axios.get(`${BACK_END_URL}/CarService/ServiceOrderAcceptedByClient?id=${id}&description=${description}`, {
            headers: {
