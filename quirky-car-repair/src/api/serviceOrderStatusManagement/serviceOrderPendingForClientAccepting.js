...
export async function serviceOrderPendingForClientAccepting(id, description) {
    const user = sessionStorage["user"] ? JSON.parse(sessionStorage["user"]) : [];
    try {
        const response = await axios.get(`${BACK_END_URL}/CarService/ServiceOrderPendingForClientAccepting?id=${id}&description=${description}`, {
            headers: {
