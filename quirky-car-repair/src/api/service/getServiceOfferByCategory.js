...
export async function getServiceOfferByCategory(id) {
    const user = sessionStorage["user"] ? JSON.parse(sessionStorage["user"]) : [];
    try {
        const response = await axios.get(`${BACK_END_URL}/CarService/GetServiceOfferByMainCategory?mainCategoryId=${id}`, {
            headers: {
